...
\subsection{Rigorous Performance Verification}
To ensure the reported high accuracy is not a result of a favorable train-test split ("lucky split"), we implemented a rigorous validation protocol:

\begin{itemize}
    \item \textbf{5-Fold Cross-Validation}: The entire dataset was shuffled and partitioned into 5 balanced folds. The model was trained and evaluated iteratively, ensuring that every image was used for validation exactly once. The final reported accuracy is the mean of these 5 independent runs, providing a statistically significant measure of performance.
    \item \textbf{Test-Time Augmentation (TTA)}: During inference, we employ a TTA strategy where prediction is averaged across multiple views (original, horizontal flip, and slight rotations). This mitigates the risk of misclassification due to orientation artifacts.
    \item \textbf{Label Smoothing}: To prevent the model from becoming overconfident and to improve generalization, we applied Label Smoothing ($\epsilon=0.1$) to the Cross-Entropy Loss. This encourages the model to learn more robust feature representations rather than fitting to hard class boundaries.
\end{itemize}
...
\midrule
Gaussian Noise ($\sigma=0.01$) & \textbf{0.56} \\
...
However, our robustness testing revealed a critical trade-off. While EfficientNet is highly stable under geometric rotation (0.98), its sensitivity to Gaussian noise (0.56) is higher than that of ResNet. This suggests that the model's high precision is partially dependent on fine-grained pixel relationships that are easily disrupted by sensor noise.
